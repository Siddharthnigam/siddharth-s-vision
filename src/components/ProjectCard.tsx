...
const ProjectCard = ({ project, onSelect, compact = false }: ProjectCardProps) => (
  <motion.div
    whileHover={{ y: -8 }}
  ...
  >
    <div className={`${compact ? "aspect-[2/1]" : "aspect-video"} overflow-hidden`}>
      <img
        src={project.image}
        alt={project.title}
    ...
    <div className={compact ? "p-4" : "p-6"}>
      <div className="flex justify-between items-start mb-3">
        <div>
          ...
          <h3 className={`${compact ? "text-lg" : "text-xl"} font-medium text-foreground`}>{project.title}</h3>
        </div>
        <a
          href={project.link}
          target="_blank"
      ...
      <div className="flex flex-wrap gap-2">
        {project.tech.slice(0, compact ? 3 : undefined).map((t) => (
          <span
            key={t}
